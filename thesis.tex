...
\newglossaryentry{S}{%
name={S},%
description={request loanAmount}%
}
\newglossaryentry{t}{%
name={t},%
...
\newglossaryentry{tEIT}{%
name={tEIT},%
description={totalEcoIndexTruck}%
}
\newglossaryentry{qIT}{%
name={qIT},%
...
% Would rather use smth like consumer technology instead of IT, we do hardware not information
...
% maximize net worth instead of profit
...
\label{SettingStory}
% I don't understand what is meant by modality
% ... one-man business with you as the only employee ...
% corporation instead of corporate
Capitalism X is created relying on a progressive game-play modality, meaning that the game will start as a one-man business and then evolve step by step, by employing, producing, expanding, buying and selling until the end goal is achieved, which is to transform the initial company into a large corporate. 
...
Regarding the time-frame, our game simulation is going to be set between $1990$ and $2017$. We have collected data for several factors that influence the economy and each year in the real world is represented by $365$ seconds.

% This is not accurate. Parameters can change daily, monthly, quarterly, yearly or asynchronously through user inputs.
This results in a change of parameters in the game every $6$ minutes, in order to represent yearly real data. This means that the total number of hours you can play Capitalism X is approximately $2.7$ hours. 

% That's probably better in the ideas section
...
It is recommended to have a look at the tutorial provided 
%after the registration process 
if you have not played Capitalism X before. Also, it might be very helpful to read this game manual, as it gives you a couple of hints about how to handle some situations. The tutorial is a short video that gives you an overview about the different buildings and views in the game. Also, it provides you with step-by-step instructions on how to perform the most important activities.
% We don't have a video, I find this confusing for Heiner --> idea list
...
% would replace "revisit" here
Throughout the game, time passes starting from January 1st, 1990 on. You can revisit this time at the top right corner of the screen and can be seen in picture \ref{fig:time}.
\begin{figure} [!htbp]
    \centering
...
% manufactured goods first instead of first manufactured goods
You as the CEO decide which products you want to offer. In Capitalism X, there are two different types of products. Those two different groups are first manufactured goods, and second, resale products that are simply bought as a whole and resold under the company brand.
...
% Replace with recent screenshot from fullGraph_shrinked
...
\begin{figure}[H]
    \includegraphics[angle=90, scale = 0.32]{fullGraph.pdf}
...
This project at hand is a concept of a business simulation game called "Capitalism X". The first two chapters constitute a user manual for a potential player of the game, explaining what to expect from the  game, what role the player has and how the game is meant to be played. The third chapter addresses the game mechanics which are mostly covered and tested by the game prototype. 

However, the concept as described in chapter 3 contains more functionality than the prototype currently provides. When some functionality is not covered in the prototype, this is mentioned in the respective simulations. All simulations are consolidated in the overall full Graph\ref{fig:OverallGraph}, which can be found in figure xy. 
Throughout the development process, multiple KPIs were designed, which fit into the different game simulations. They are aligned with each other, sampling the real-world in most cases and where 
applicable. In the end this resulted in a sound concept. 

Some trivial facts of the project like the course, personal opinion

\section{Future Work}
\label{sec:future}
There were far more ideas than could be captured during the development of the game. However, they should not be over-passed as the game can be further extended. This may be the case in a follow-up project. Finally, the game concept might also be sold to a gaming company which is willing to fully program and publish the game. 

The original concept was introducing another phase of the game, which would take place from 2017-2030. In this following phase the company would evolve to a big player like Google or Facebook or Amazon, and the goal is to set an environment which allows to control every aspect of people’s lives. 
We also came up with different modes of game-play. Firstly, the player would start with a normal desktop game, in the second stage the multiplayer modality would be introduced and as the game progresses there would be the possibility to extend to virtual reality. 

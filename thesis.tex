...
\newglossaryentry{aOA_{cS}}{%
name={aOA_{cS}},%
description={averageOverallAppeal_customerSatisfaction}%
}
...
\label{sec:link}
A functioning business simulation requires that all mechanics play well together. A graphical representation contains all information in one place. This helps when implementing the prototype and writing the report. The so called 'fullGraph' is given below after the legend that describes its structure. 
Also, it should be assumed we can use the game time in our calculations. With $t_d$ we denote the time of elapsed days, whereas $t_y$ is the number of years elapsed.

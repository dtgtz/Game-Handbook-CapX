...
Regarding the time-frame our game simulation is going to be set between $1990$ and $2017$. We have collected data for several factors that influence economy and each year in the real world would be represented by $365$ seconds.
This results in a change of parameters in the game every $6$ minutes, in order to represent yearly real data. This means that the total number of hours you can play CapitalismX is approximately $2.7$ hours. 
...
\label{sec:link}
Maybe rename to overview of mechanics
A functioning business simulation requires that all mechanics play well together. A graphical representation contains all information in one place. This helps when implementing the prototype and writing the report. In addition, such an overview supports the alignment of the different in-game simulations. The so called 'fullGraph' is shown below in figure xy. 
It is also assumed that we can use the game time in our calculations. With $t_d$ we denote the time of elapsed days, whereas $t_y$ is the number of years elapsed. Time starts at 1st January 1990. 

NEW PICTURE!!!
\begin{figure}[h]
    \centering
    \includegraphics[scale = 0.7]{fullGraph_legend.pdf}
    \caption{Legend Overall Graph including all parts of CapitalismX}
    \label{fig:OverallGraphLegend}
\end{figure}
+ set to beginning of chapter 3 --> then chapter needs new name
\begin{landscape}
\begin{figure}
    \centering
...
\begin{figure}
    \raggedright
    \includegraphics[scale = 0.32]{fullGraph_shrinked.pdf}
    \caption{Overall Graph including all parts of CapitalismX2}
    \label{fig:OverallGraph2}
\end{figure}
\end{landscape}
...
\chapter{Conclusion}
This project at hand is a concept of a business simulation game called "CapitalismX". The first two chapters constitute a user manual for a potential player of the game, explaining what to expect from the  game, what role the player has and how the game is meant to be played. The third chapter addresses the game mechanics which are mostly covered and tested by the game prototype. 
...
\section{Future Work}
\label{sec:future}
There were far more ideas than could be captured during the development of the game. However, they should not be over-passed as the game can be further extended. This may be the case in a follow-up project. Finally, the game concept might also be sold to a gaming company which is willing to fully program and publish the game. The original concept was introducing another phase of the game, which would take place from 2017-2030. In this following phase the company would evolve to a big player like Google or Facebook or Amazon, and the goal is to set an environment which allows to control every aspect of people’s lives. 

We also came up with different modalities of game-play. Firstly, the player would start with a normal desktop game, in the second stage the multiplayer modality would be introduced and as the game progresses there would be the possibility to extend to virtual reality. 
...
\textbf{Events}           & More external events can included in the game based on the added features                                                                                                                        \\ \hline
\end{tabular}

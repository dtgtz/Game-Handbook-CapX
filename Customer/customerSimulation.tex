\section{Customer Simulation} \label{sec:customsim}
%Maike
...
\begin{equation}
overallAppeal = \dfrac{productAppeal}{priceAppeal} \times 100
\end{equation}
...
\label{cSCalc}
    & cS(oA, tSQ, lI, cI, eB) \\
    & = tanh(\dfrac{(0,5 \times oA + 0,2 \times tSQ + 0,1 \times lI + 0,15 \times cI + 0,05 \times eB)}{value}) %value must be defined!
\end{aligned}    
\end{equation}
% tanh((0,5*oA + 0,2*tSQ + 0,1*lI + 0,15*cI + 0,05*eB)/1)
% tanh((0,5*3500 + 0,2*100 + 0,1*100 + 0,15*100 + 0,05*100)/1)

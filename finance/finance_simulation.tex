...
% @Salih: ist das jetzt noch so korrekt? Weil die Taxes bezahlt er ja täglich, das passt ja also, aber mit den loan interests war das jetzt wie?
% @Nike: Im Finance dashboard werden ja alle Kennzahlen täglich upgedated. Wir haben geschrieben, dass unser balance sheet monatlich geupdated wird. Danach wird die NEt worth Formel aufgestellt und wir schreiben, dass diese ad hoc geupdated wird. Sollten wir uns darauf einigen, dass alle Formeln in der Implementierung auf den tag berechnet werden? Dann wäre deine Frage bzgl. tax update und loan interests geklärt oder?
:o
\begin{equation}
\label{func:netWorth}

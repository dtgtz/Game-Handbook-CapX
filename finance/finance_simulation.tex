...
\begin{itemize}
    \item Goods sold, displayed by the \textit{totalSalesRevenues}, which can be derived by multiplying a product’s sales price \textit{(sP)} with the amount of units sold \textit{(sF)}. 
    \begin{equation}
        totalSalesRevenue~(\gls{tSR}) = \sum_{p \in P}(sF \cdot sP) 
    \end{equation}
    \item Assets Sold (\textit{assetsSold}). %rewrite all this item and all subitems.

...
The internal logistics fleet consists only of purchased trucks.The quality of the company's own logistics is calculated based on the characteristics of the single trucks. 
...
\begin{itemize}
    ...
    \item A \textit{purchasePrice} that depends on the quality and environmental friendliness of the truck. The base price for a truck is 100,000 CapCoins \cite{televon_wie_nodate}.
    ...
    \item And a \textit{depreciationRate} that is linear and amounts to 9 years for each truck \cite{noauthor_amtliche_nodate}.
\end{itemize}
...
\begin{aligned}
    ...
    Internal Logistic Index (\gls{iLI}) = eF \times 0,2 + qF \times 0,8 \\
\end{aligned}
\end{equation}
...
tDC = 
\begin{cases}
    (( dP / 1000) \times fCD) + ( dP \times vCD) & \text{if} dP = cF\\
    (\lceil (dP / 1000) \rceil \times fCD ) + ( dP \times vCD) & \text{if} dP < cF\\
    ( nT \times fCD ) + ( nT \times1000 \times vCD ) + cED & \text{otherwise}\\
\end{cases}
\end{aligned}
\end{equation}
\\
...
eLI = 
    \begin{cases}
     (rLP\times0,5 + 0,5\times(qLP\times0,8 + eIP\times0,2)) & \text{if} rLP \leq 40\\
     (rLP\times0,4 + 0,6\times(qLP\times0,8 + eIP\times0,2)) & \text{otherwise} \\
    \end{cases}
\end{aligned}
\end{equation}
...
lI = 
\begin{cases}
     ((( dP – cF) \times eLI ) + ( cF \times iLI )) & \text{if} dP > cF \\
     cF \times iLI & \text{if} dP = cF\\
     dP \times iLI & \text{otherwise} \\
\end{cases}
\end{aligned}
\end{equation}

The \textit{totalLogisticCost} (\gls{tLC}) consists of the monthly \textit{logisticCosts} and the daily \textit{deliveryCost}. 
\begin{equation}
    tLC = (lC/30) + dC
\end{equation}
